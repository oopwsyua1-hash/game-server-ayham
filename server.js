...
app.post('/register', async (req, res) => {
  ...
  if (!email ||!username ||!password) return res.json({ success: false, message: 'عبي كل الحقول' });
 ...
 .select();
  ...
  if (error) return res.json({ success: false, message: 'الايميل مستخدم او خطأ' });
...
app.post('/login', async (req, res) => {
  ...
  if (error ||!data) return res.json({ success: false, message: 'الايميل غلط' });

  const match = await bcrypt.compare(password, data.password);
  if (!match) return res.json({ success: false, message: 'كلمة السر غلط' });
