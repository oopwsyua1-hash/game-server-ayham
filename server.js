...
app.post('/api/register', async (req, res) => {
  try {
    const { username, password, email, birthDate, gender, country } = req.body;
    const existingUser = await User.findOne({ username });
    if (existingUser) return res.status(400).json({ message: 'اسم المستخدم موجود مسبقاً' });
    if (email) {
      const existingEmail = await User.findOne({ email });
      if (existingEmail) return res.status(400).json({ message: 'الايميل مستخدم مسبقاً' });
    }
    const hashedPassword = await bcrypt.hash(password, 10);
    const user = await User.create({ username, password: hashedPassword, email: email || '', birthDate: birthDate || '', gender: gender || '' });
...
app.get('/api/user/:userId', async (req, res) => {
  try {
    ...
    const user = await User.findOneAndUpdate({ userId }, { $inc: { visitors: 1 } }, { new: true }).select('-password');
...
app.post('/api/agency/create', auth, async (req, res) => {
  try {
    const { name } = req.body;
    const agency = await Agency.create({ name, owner: req.userId, members: [{ user: req.userId, role: 'owner' }] });
...
app.get('/api/agency/my', auth, async (req, res) => {
  try {
    const user = await User.findById(req.userId).populate({ path: 'agency', populate: { path: 'members.user', select: 'username avatar userId' } });
...
app.post('/api/agency/leave', auth, async (req, res) => {
  try {
    ...
    await Agency.updateOne({ _id: user.agency }, { $pull: { members: { user: req.userId } } });
...
app.post('/api/agency/update', auth, async (req, res) => {
  try {
    const user = await User.findById(req.userId);
    if (!user.agency || (user.agencyRole!== 'owner' && user.agencyRole!== 'admin')) return res.status(403).json({ message: 'غير مصرح' });
...
app.post('/api/agency/kick', auth, async (req, res) => {
  try {
    ...
    const user = await User.findById(req.userId);
    if (!user.agency || (user.agencyRole!== 'owner' && user.agencyRole!== 'admin')) return res.status(403).json({ message: 'غير مصرح' });
    await Agency.updateOne({ _id: user.agency }, { $pull: { members: { user: userId } } });
...
app.post('/api/agency/promote', auth, async (req, res) => {
  try {
    const { userId } = req.body;
    const user = await User.findById(req.userId);
    if (!user.agency || user.agencyRole!== 'owner') return res.status(403).json({ message: 'المالك فقط يقدر يرقي' });
    await Agency.updateOne({ _id: user.agency, 'members.user': userId }, { $set: { 'members.$.role': 'admin' } });
...
app.get('/api/admin/online', auth, adminOnly, async (req, res) => {
  try {
    ...
    const onlineUsers = await User.find({ lastSeen: { $gte: twoMinutesAgo } }).select('username userId avatar lastSeen');
...
app.post('/api/admin/remove-admin/:userId', auth, adminOnly, async (req, res) => {
  try {
    ...
    if (targetUser._id.toString() === req.userId) return res.status(400).json({ message: 'ما بتقدر تسحب الصلاحية من حالك' });
