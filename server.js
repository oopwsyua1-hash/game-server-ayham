...
const rooms = { '10000': { users: [], mics: Array(20).fill(null), admins: [] } };
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        if (exists) return res.json({ error: 'الايميل مستخدم' });
...
io.on('connection', (socket) => {
    socket.on('joinRoom', async ({ roomId, token }) => {
        try {
            ...
            rooms[roomId].users.push({ socketId: socket.id, userId: user._id, username: user.username, level: user.level, vip: user.vip });
    ...
    socket.on('adminAction', async ({ action, targetUserId, roomId }) => {
        ...
        if (action === 'KICK') {
            const target = rooms[roomId].users.find(u => u.userId == targetUserId);
            if (target) io.sockets.sockets.get(target.socketId)?.disconnect();
        }
        if (action === 'CLOSE_MIC') {
            const idx = rooms[roomId].mics.findIndex(m => m && rooms[roomId].users.find(u => u.socketId === m && u.userId == targetUserId));
            if (idx!== -1) rooms[roomId].mics[idx] = 'closed';
            io.to(roomId).emit('roomUpdate', rooms[roomId]);
        }
        if (action === 'OPEN_MIC') {
            const idx = rooms[roomId].mics.findIndex(m => m === 'closed');
            if (idx!== -1) rooms[roomId].mics[idx] = null;
            io.to(roomId).emit('roomUpdate', rooms[roomId]);
        }
    });
